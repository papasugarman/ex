...
contract Vend{
    ...
    function exchange() public returns (bool){
        ...
        toMint-=amount;
        totalMinted+=amount;
        completedOrders.push(orders[msg.sender].path);
    ...
    function getOrderDetails() public view returns(uint, string memory,string memory,string memory, string memory){
        return (orders[msg.sender].xexAmount,
                orders[msg.sender].add_btc,  orders[msg.sender].add_eth,
                orders[msg.sender].bal_btc, orders[msg.sender].bal_eth);
    }
    ...
    string memory _add_btc, string memory _add_eth, 
    string memory _bal_btc, string memory _bal_eth) 
    public returns (bool){
        require(EX(EXBase).getAccAt(5)==msg.sender || EX(EXBase).getAccAt(0)==msg.sender); //
